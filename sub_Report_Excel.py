...
def Create_IBIS_Excel_Report():
	try:
		...
		for row in sub_DB.IBIS_ResultForm._DataGridView.Rows:
			# Create IBIS optimization detailed sheet
			xlsheet_table = xlbook.Worksheets.Add()
			xlsheet_table.Name = "case" + str(row.Cells[0].Value)

			# Create Column
			xlsheet_table.Cells[1,1] = "%s & %s" % (row.Cells[1].Value, row.Cells[2].Value)
			xlsheet_table.Cells[1,2] = "Analyze Group"
			xlsheet_table.Cells[1,3] = "Width [ps]"		
			xlsheet_table.Cells[1,4] = "Timing Margin [ps]"
			xlsheet_table.Cells[1,5] = "Width [%]"		
			...
			# Add Rows - Eye Measurement Results
			row_idx = 2
			for key in sorted(Eye_Measure_Results[xlsheet_table.Name].keys()):
				xlsheet_table.Cells[row_idx,1] = key
				xlsheet_table.Cells[row_idx,2] = 'None'
				xlsheet_table.Cells[row_idx,3] = Eye_Measure_Results[xlsheet_table.Name][key][0]
				xlsheet_table.Cells[row_idx,4] = Eye_Measure_Results[xlsheet_table.Name][key][2]
				
				xlsheet_table.Cells[row_idx,5] = round((Eye_Measure_Results[xlsheet_table.Name][key][0]/sub_DB.UI)*100, 1)	# Width [%]
				xlsheet_table.Cells[row_idx,6] = round((Eye_Measure_Results[xlsheet_table.Name][key][2]/sub_DB.UI)*100, 1)	# Margin [%]
				xlsheet_table.Cells[row_idx,7] = round(row.Cells[8].Value, 1) # Vref
				row_idx += 1
			row_idx -= 1
			Log("		(Add Data) = Done")
		...
		############################
		# Create Eye Diagram Sheet #
		############################
		# Create Eye Diagram Image Report Worksheet
		xlsheet_table = xlbook.Worksheets.Add()
		xlsheet_table.Name = "EYE Diagrams"
		
		imgw = int(sub_DB.IBIS_ResultForm._TextBox_ImageWidth.Text)
		imgh = imgw / 5 * 4
		for i in range(0, len(sub_DB.Excel_Img_File)):
			j = (i-4*(int(i/4)))*imgw
			k = int(i/4)*imgh
		...
		#########################
		# Create IBIS optimization overview sheet
		xlsheet_table = xlbook.Worksheets.Add()
		xlsheet_table.Name = "Overview"

		# Create Column
		xlsheet_table.Cells[1,1] = "Sim. Case #"
		xlsheet_table.Cells[1,2] = "Tx IBIS Model"
		xlsheet_table.Cells[1,3] = "Rx IBIS Model"
		xlsheet_table.Cells[1,4] = "Average Width"
		xlsheet_table.Cells[1,5] = "Average Margin"
		xlsheet_table.Cells[1,6] = "Worst Width"
		xlsheet_table.Cells[1,7] = "Worst Margin"
		xlsheet_table.Cells[1,8] = "Vref [mV]"
		Log("		(Create Column) = Done")

		# Create Column Range
		Col_Header = xlsheet_table.Range[xlsheet_table.Cells[1, 1], xlsheet_table.Cells[1, 8]]
		...
		row_idx = 2
		for row in sub_DB.IBIS_ResultForm._DataGridView.Rows:
			xlsheet_table.Cells[row_idx,1] = row.Cells[0].Value
			xlsheet_table.Cells[row_idx,2] = row.Cells[1].Value
			xlsheet_table.Cells[row_idx,3] = row.Cells[2].Value
			xlsheet_table.Cells[row_idx,4] = row.Cells[4].Value
			xlsheet_table.Cells[row_idx,5] = row.Cells[5].Value
			xlsheet_table.Cells[row_idx,6] = row.Cells[6].Value
			xlsheet_table.Cells[row_idx,7] = row.Cells[7].Value
			xlsheet_table.Cells[row_idx,8] = row.Cells[8].Value
			row_idx += 1
		row_idx -= 1
		Log("		(Add Data) = Done")
		...
		# Create Data Range
		Data_Cell = xlsheet_table.Range[xlsheet_table.Cells[2, 2], xlsheet_table.Cells[row_idx, 8]]
		Data_Cell.Borders.LineStyle = Excel.XlLineStyle.xlContinuous
		Data_Cell.Borders.Weight = Excel.XlBorderWeight.xlThin
...
def Create_IBIS_Excel_Report_wo_fig():
	try:
		xlApp = Excel.ApplicationClass()
		Report_Name = ""
		for item in sub_DB.Eye_Form._CheckedListBox_ReportName.CheckedItems:
